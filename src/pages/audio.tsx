...
export default function AudioMerger() {
  ...
  useEffect(() => {
    return () => loadedAudios.forEach(({ url }) => URL.revokeObjectURL(url));
  }, [loadedAudios]);
                      ...
                      <div className="space-y-4">
                        ...
                        >
                          {loadedAudios.map(({ duration, id, name, size, url }) => (
                            <div key={id} className="flex gap-2 items-center px-3 py-2 border rounded-xl shadow-sm text-sm">
                              <ReorderIcon />
                              <button
                              ...
                              <div className="min-w-1/3 max-w-1/2">
                      ...
                      <div className="space-y-4">
                        ...
                        >
                          {advancedSelections.map((selection) => (
                            <div key={selection.id} className="flex gap-2 items-center px-2.5 py-2 border rounded-xl shadow-sm text-sm">
                              <ReorderIcon />
                              <select
                                value={selection.audioIndex}
                                onChange={(e) => handleAdvancedUpdate(selection.id, { audioIndex: +e.target.value })}
                                className="max-w-xs p-2 border border-slate-300 dark:border-slate-600 rounded bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white"
                              >
                              ...
                              <input
                                ...
                                className="w-32 border border-slate-300 dark:border-slate-600 rounded p-2 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white"
                                step={0.1}
                                min={0}
                              />
                              <button
